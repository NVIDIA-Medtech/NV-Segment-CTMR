...
class NormalizeMixin(metaclass=abc.ABCMeta):
    ...
    @staticmethod
    def skull_stripped_foreground(image: intnormt.ImageLike, /, *, background_threshold: float = 1e-6) -> intnormt.ImageLike:
        if image.min() < 0.0:
    ...
    def _get_mask(
        ...
        *,
        ...
        if mask is None:
            mask = self.skull_stripped_foreground(image, background_threshold=background_threshold)
...
class NormalizeCLIMixin(NormalizeMixin, intnormcli.CLIMixin, metaclass=abc.ABCMeta):
    def normalize_from_filename(
        ...
        *,
        ...
        normalized = typing.cast(mioi.Image, self.normalize_image(image, mask, modality=modality))
    ...
    def get_parent_parser(
        ...
        **kwargs: typing.Any,
        ...
        parser = super().get_parent_parser(desc, valid_modalities=valid_modalities, **kwargs)
        parser.add_argument(
            "-p",
            "--plot-histogram",
            action="store_true",
...
class DirectoryNormalizeCLI(SampleNormalizeCLIMixin, intnormcli.DirectoryCLI, metaclass=abc.ABCMeta):
    def fit(
        self,
        images: ImageSeq,
        /,
